  ...
  return (
    <div className="space-y-2">
      ...
      <div className="w-full border border-gray-300 rounded-md p-3 text-sm cursor-not-allowed text-gray-400">
        <div className="bg-[#EFEFEF] p-2 rounded w-fit">{text}</div>
      </div>
    </div>
  );
}
...
export default function AddActivityReport() {
  ...
  const [formData, setFormData] = useState<
    Omit<ActivityReportPayload, "activityReportId" | "activityId">
  >({
    ...
    <div className="w-156">
      <CardComponent>
        ...
        <div className="space-y-6 my-6">
          <PrefilledInput
            text={selectedActivityToPrefillForm.activityStatement}
            label="Activity Statement"
          />
          <PrefilledInput
            text={selectedActivityToPrefillForm.outputStatement}
            label="Linked to Output"
          />
          <PrefilledInput
            text={`₦ ${selectedActivityToPrefillForm.activityTotalBudget}`}
            label="Activity Total Budget"
          />
          <PrefilledInput
            text={selectedActivityToPrefillForm.responsiblePerson}
          ...
          <PrefilledInput
            text={selectedActivityToPrefillForm.subActivity}
            label="Type"
          />
          <PrefilledInput
            text={formatDate(
              selectedActivityToPrefillForm.deliveryDate,
              "date-only",
            )}
            label="Delivery Date"
          />
          <PrefilledInput
            text={selectedActivityToPrefillForm.descriptionAction}
            label="Description"
          />
          <p className="text-gray-900 font-bold text-base">
            Action 1: Percent completion (%)
          </p>
