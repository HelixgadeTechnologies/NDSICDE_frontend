...
export default function ProjectOutcomeModal({
  ...
  // form data - using consistent field names
  const [formData, setFormData] = useState({
    outcomeStatement: "",
    outcomeType: "",
    impactId: "",
    thematicAreas: "", // Changed to match payload
  });
  ...
  const fetchImpact = async () => {
    ...
    try {
      ...
      console.log("Impacts API response:", response.data);
      ...
      const transformedOptions: DropdownOption[] = impactsData.map(
        (impact: any) => ({
          label: impact.statement || impact.impactStatement || `Impact`,
          value: impact.impactId || impact.id || "",
        })
      );
