...
export default function InternalMemorandum({
  ...
  return (
    <div className="w-full">
      ...
      <div className="flex flex-col md:flex-row md:justify-between items-center md:items-start gap-4 md:gap-0 mb-6 text-black border-b-2 border-gray-300 pb-4">
      ...
      <div className="border border-gray-300 text-xs text-gray-500 mb-8 select-none">
        {/* Row 1 */}
        <div className="flex flex-col sm:flex-row border-b border-gray-300">
          <div className="w-full sm:w-1/2 p-2.5 border-b sm:border-b-0 sm:border-r border-gray-300 flex items-center gap-2 bg-gray-50/55">
            <span className="font-bold uppercase tracking-wider text-[10px] w-16 sm:w-24 shrink-0">FROM:</span>
            <span className="text-gray-500 font-semibold text-xs">{staff || "N/A"}</span>
          </div>
          <div className="w-full sm:w-1/2 p-2.5 flex items-center gap-2 bg-gray-50/55">
            ...
            <span className="text-gray-500 font-bold text-xs">{requestDate || "N/A"}</span>
          </div>
        </div>
        ...
        <div className="flex flex-col sm:flex-row border-b border-gray-300">
          <div className="w-full sm:w-1/2 p-2.5 border-b sm:border-b-0 sm:border-r border-gray-300 flex items-center gap-2 bg-gray-50/55">
            ...
            <span className="text-gray-500 font-semibold text-xs">Country Director/ Budget Holder</span>
          </div>
          <div className="w-full sm:w-1/2 p-2.5 flex items-center gap-2">
            ...
            {isReadOnly ? (
              <span className="text-gray-500 font-bold text-xs">{budgetName || "N/A"}</span>
            ) : (
              <input
                type="text"
                className="flex-1 min-w-0 bg-transparent border-none outline-none focus:ring-0 p-0 text-gray-500 font-bold text-xs placeholder:text-gray-400 placeholder:font-normal"
                value={budgetName || ""}
        ...
        <div className="flex flex-col sm:flex-row">
          <div className="w-full sm:w-1/2 p-2.5 border-b sm:border-b-0 sm:border-r border-gray-300 flex items-center gap-2 bg-gray-50/55">
            ...
            <span className="text-gray-500 font-semibold text-xs">Finance Manager</span>
          </div>
          <div className="w-full sm:w-1/2 p-2.5 flex items-center gap-2">
            <span className="font-bold uppercase tracking-wider text-[10px] w-20 sm:w-24 shrink-0">BUDGET CODE:</span>
            {isReadOnly ? (
              <span className="text-gray-500 font-bold text-xs">{budgetCode || "N/A"}</span>
            ) : (
              <input
                type="text"
                className="flex-1 min-w-0 bg-transparent border-none outline-none focus:ring-0 p-0 text-gray-500 font-bold text-xs placeholder:text-gray-400 placeholder:font-normal"
